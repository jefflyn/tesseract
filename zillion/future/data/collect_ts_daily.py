...
def add_daily(ts_codes=None):
    ...
    for code in ts_codes:
        ...
        for index, row in df_data.iterrows():
            ...
            open = row['close'] if math.isnan(row['open']) else row['open']
            high = row['close'] if math.isnan(row['high']) else row['high']
            low = row['close'] if math.isnan(row['low']) else row['low']
            data_list.append([row['ts_code'], row['trade_date'], pre_close, row['pre_settle'], open,
                              high, low, row['close'], row['settle'], row['change1'],
                              row['change2'], row['vol'], row['amount'], row['oi'], row['oi_chg'], date_util.now()])
...
if __name__ == '__main__':
    ...
    ts_code_list = ['EB2208.DCE','LU2208.INE']
