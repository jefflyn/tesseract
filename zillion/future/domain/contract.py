...
def update_hl(code, low=None, low_time=None, high=None, high_time=None, update_hist=False):
    ...
    if update_hist:
        if low is not None and low_time is not None:
            sql = "update contract set h_low=%f, h_low_time='%s', update_time=now() where code='%s' and h_low>%f"
            result = cursor.execute(sql % (low, low_time, code, low))
        if high is not None and high_time is not None:
            sql = "update contract set h_high=%f, h_high_time='%s', update_time=now() where code='%s and h_high<%f'"
            result += cursor.execute(sql % (high, high_time, code, high))
    else:
        if low is not None and low_time is not None:
            sql = "update contract set low=%f, low_time='%s', update_time=now() where code='%s' and low>%f"
            result = cursor.execute(sql % (low, low_time, code, low))
        if high is not None and high_time is not None:
            sql = "update contract set high=%f, high_time='%s', update_time=now() where code='%s' and high<%f"
            result += cursor.execute(sql % (high, high_time, code, high))
    if result > 0:
