...
def get_period_change(period_k=None):
    ...
    last_begin_close = first.ix[first.index.get_values()[0], 'close'] if (period_k is not None and len(period_k) > 0) else 0
    last_end_close = last.ix[last.index.get_values()[0], 'close'] if (period_k is not None and len(period_k) > 0) else 0
    last_change = 0
    if last_begin_close > 0:
        last_change = round((last_end_close - last_begin_close) / last_begin_close * 100, 2)
    return last_change

if __name__ == '__main__':
    print('start at', _dateutil.now)
    ...
    for index, row in basics.iterrows():
        ...
        this_month_k = target_k_data[(target_k_data.date >= this_week_start) & (target_k_data.date <= this_week_end)]
    ...
    result_df = pd.DataFrame(result_list, columns=['代码','名称','行业','地区','上周涨幅','本周涨幅','上月涨幅','本月涨幅','上季涨幅','本季涨幅','去年涨幅','今年涨幅'])
    result_df = result_df.sort_values('本周涨幅', ascending=False)
    _datautils.to_db(result_df, 'change_stat')
    print('end at', _dateutil.now)
