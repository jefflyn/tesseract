...
select * from hist_trade_day where code=300594;
-- 验证每天trade data etl
select trade_date, count(1) from hist_trade_day where trade_date >= '2019-06-01' group by trade_date order by trade_date desc;
select trade_date, count(1) from hist_index_day where trade_date >= '2019-06-01' group by trade_date order by trade_date desc;
-- daily market data
...
select * from basic order by code desc;
...
select * from hist_index_day where ts_code='399001.SZ' and trade_date='2019-02-15' order by trade_date desc;
...
select * from select_result_all where name not like '%ST%' and list_date < 20180718;
...
select * from select_result_wave where name like '%东方创业%';
