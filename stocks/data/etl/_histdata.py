...
def hist_data_week_extract(n=8):
    """
    run every week
    get_hist_data func use for getting 6 weeks' change data
    ...
    for code in codes:
        histdf = ts.get_hist_data(code=code, ktype='W', start=dconst.FIRST_DAY_6_WEEK)
    ...
    _dt.to_db(hist_results, 'hist_data_week')
    _dt.to_db(change_results, 'hist_change_week')
